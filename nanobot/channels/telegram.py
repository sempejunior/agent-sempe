...
class _ConflictFilter(logging.Filter):
    """Suppress Telegram Conflict tracebacks from the library's internal retry loop."""
...
class TelegramChannel(BaseChannel):
    ...
    async def _wait_for_exclusive_access(self) -> None:
        """Wait until any previous polling session has expired on Telegram's side."""
        for attempt in range(12):
            try:
                ...
                if attempt == 0:
                    logger.info("Waiting for previous Telegram session to expire...")
                await asyncio.sleep(3)
            except Exception:
                return
        logger.warning("Previous Telegram session still active after retries, starting anyway")
    ...
    async def _on_error(self, update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Log handler errors. Conflict errors are suppressed (handled by retry logic)."""
