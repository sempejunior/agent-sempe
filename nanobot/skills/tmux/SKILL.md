...
Use tmux only when you need an interactive TTY. For delegating a coding task to an
agent CLI, use the `code_agent` tool instead — it runs the CLI headless, keeps the
log on disk and survives a timeout. Driving a CLI through tmux is a last resort.
