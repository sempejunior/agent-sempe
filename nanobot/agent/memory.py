...
if TYPE_CHECKING:
    from nanobot.db.repositories import ClientMemoryRepository, MemoryRepository
...
class ClientMemoryStore:
    """Per-client memory store using ClientMemoryRepository. DB-only."""
    ...
    async def consolidate(
        ...
        *,
        ...
        if archive_all:
            ...
            keep_count = 0
            logger.info(
                "Client memory consolidation (archive_all): {} messages",
            ...
            if not old_messages:
                return True
            logger.info(
                "Client memory consolidation: {} to consolidate, {} keep",
                len(old_messages),
                keep_count,
            )
...
{chr(10).join(lines)}"""
        ...
        try:
            ...
            if not response.has_tool_calls:
                logger.warning(
                    "Client memory consolidation: LLM did not call save_memory, skipping"
                )
                return False
            ...
            if not isinstance(args, dict):
                logger.warning(
                    "Client memory consolidation: unexpected arguments type {}",
                    type(args).__name__,
                )
                return False
            ...
            logger.info(
                "Client memory consolidation done: {} messages, last_consolidated={}",
                len(session.messages),
                session.last_consolidated,
            )
            return True
        except Exception:
            logger.exception("Client memory consolidation failed")
            return False
